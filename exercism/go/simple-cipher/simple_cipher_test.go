...
func TestRandomShift(t *testing.T) {
	c := RandomShift()
	input := "secret_string"
	expectedDecoded := "secretstring"
	actualDecoded := c.Decode(c.Encode(input))
	if actualDecoded != expectedDecoded {
		t.Errorf("Unexpected decoded message.\nInput: '%s'\nDecoded:%s",
			input, actualDecoded)
	}
}

func TestRandomVigenere(t *testing.T) {
	c := RandomVigenere()
	input := "secret_string"
	expectedDecoded := "secretstring"
	actualDecoded := c.Decode(c.Encode(input))
